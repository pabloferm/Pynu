#!/usr/bin/env python3
"""
IceCube Upgrade Standard Sensitivity Scan Row Worker (for SLURM array jobs)
...
No hypersurface corrections (not in data release).
No muon background (negligible per README).
...
Usage:
    python run_icup_sensitivity_row_worker.py --row-idx 0 --n-dm 41 --n-s23 41 --output-dir DIR
"""
...
def setup_pynufit(config):
    ...
    # Build Asimov = neutrino expectation (no muon background for ICUp)
...
def main():
    parser = argparse.ArgumentParser(description="ICUp sensitivity scan row worker")
    ...
    if args.config is None:
        args.config = f"{PROJECT_DIR}/Pynu/examples/AnalysisFiles/ICUp_Atm.xml"
