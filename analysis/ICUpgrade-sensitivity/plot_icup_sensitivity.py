#!/usr/bin/env python3
"""
Plot IceCube Upgrade Sensitivity Contours from 2D Profile Scan

Generates:
1. 2D contour plot in (sin^2 theta_23, Dm^2_31) plane with CL contours
2. 1D Dm^2_31 profile (marginalized over sin^2 theta_23)
3. 1D sin^2 theta_23 profile (marginalized over Dm^2_31)

Usage:
    python plot_icup_sensitivity.py --input-dir DIR [--output-dir DIR]
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Plot ICUp sensitivity contours")
...
def main():
    ...
    # =========================================================================
    # Plot 1: 2D contour plot — x=sin^2 theta_23, y=Dm^2_31
    # =========================================================================
    fig, ax = plt.subplots(1, 1, figsize=(8, 7))

    # chi2_grid shape is (n_dm, n_s23), contour(X=s23, Y=dm, Z=delta_chi2)
    ...
    plot_title = args.title if args.title else 'IceCube Upgrade 10yr Sensitivity (Asimov)'
    ax.set_title(plot_title, fontsize=16)
    ax.legend(loc='upper left', fontsize=10, framealpha=0.9)
    ax.tick_params(labelsize=12)

    plt.tight_layout()
    out_path = os.path.join(args.output_dir, "icup_sensitivity_contours.png")
    fig.savefig(out_path, dpi=150, bbox_inches='tight')
    print(f"Saved: {out_path}")
    plt.close(fig)

    # =========================================================================
    # Plot 2: 1D Dm^2_31 profile (marginalized over sin^2 theta_23)
    # =========================================================================
    ...
    ax2.set_title(r'ICUp 1D Profile: $\Delta m^2_{31}$ (marginalized over $\sin^2\theta_{23}$)', fontsize=16)
    ax2.legend(fontsize=10)
    ax2.set_ylim(bottom=-0.5)
    ax2.tick_params(labelsize=12)

    plt.tight_layout()
    out_path2 = os.path.join(args.output_dir, "icup_1d_profile_dm231.png")
    fig2.savefig(out_path2, dpi=150, bbox_inches='tight')
    print(f"Saved: {out_path2}")
    plt.close(fig2)

    # =========================================================================
    # Plot 3: 1D sin^2 theta_23 profile (marginalized over Dm^2_31)
    # =========================================================================
    ...
    ax3.set_title(r'ICUp 1D Profile: $\sin^2\theta_{23}$ (marginalized over $\Delta m^2_{31}$)', fontsize=16)
    ax3.legend(fontsize=10)
    ax3.set_ylim(bottom=-0.5)
    ax3.tick_params(labelsize=12)

    plt.tight_layout()
    out_path3 = os.path.join(args.output_dir, "icup_1d_profile_sin2theta23.png")
