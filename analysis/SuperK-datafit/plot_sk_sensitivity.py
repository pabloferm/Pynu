#!/usr/bin/env python3
"""
Plot Super-Kamiokande 2023 Sensitivity Contours from 2D Profile Scan
...
Usage:
    python plot_sk_sensitivity.py --input-dir DIR [--output-dir DIR]
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Plot SK sensitivity contours")
...
def main():
    ...
    plot_title = args.title if args.title else 'Super-K 2023 Sensitivity (Asimov)'
    ax.set_title(plot_title, fontsize=16)
    ax.legend(loc='upper left', fontsize=10, framealpha=0.9)
    ax.tick_params(labelsize=12)

    plt.tight_layout()
    out_path = os.path.join(args.output_dir, "sk_sensitivity_contours.png")
    ...
    ax2.set_title(r'SK 2023 1D Profile: $\Delta m^2_{31}$ (marginalized over $\sin^2\theta_{23}$)', fontsize=16)
    ax2.legend(fontsize=10)
    ax2.set_ylim(bottom=-0.5)
    ax2.tick_params(labelsize=12)

    plt.tight_layout()
    out_path2 = os.path.join(args.output_dir, "sk_1d_profile_dm231.png")
    ...
    ax3.set_title(r'SK 2023 1D Profile: $\sin^2\theta_{23}$ (marginalized over $\Delta m^2_{31}$)', fontsize=16)
    ax3.legend(fontsize=10)
    ax3.set_ylim(bottom=-0.5)
    ax3.tick_params(labelsize=12)

    plt.tight_layout()
    out_path3 = os.path.join(args.output_dir, "sk_1d_profile_sin2theta23.png")
