#!/bin/bash
#SBATCH -J ic_hs_sens
#SBATCH -o logs/ic_hs_sens_%a_%j.out
#SBATCH -e logs/ic_hs_sens_%a_%j.err
#SBATCH -c 4
#SBATCH --mem=16G
#SBATCH -t 0-01:00
...
OUTPUT_DIR=${OUTPUT_DIR:-${SCRIPT_DIR}/results/ic_sensitivity_hs_41x41/rows}
mkdir -p ${OUTPUT_DIR}
mkdir -p logs

python -u ${SCRIPT_DIR}/run_ic_sensitivity_row_worker.py \
    --row-idx ${SLURM_ARRAY_TASK_ID} \
    --n-dm 41 --n-s23 41 \
    --config ${SCRIPT_DIR}/IC_Atm.xml \
