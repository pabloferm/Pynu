...
def cornerPlot(x, y, X2, title=''):
    ...
    for i in range(2):
        axUp.plot([x[0], x[-1]], [levels1d[i], levels1d[i]],
                  color=colors[i], linewidth=0.5)
        axRi.plot([levels1d[i], levels1d[i]], [y[0], y[-1]],
                  color=colors[i], linewidth=0.5)
    axUp.set_ylim(0., 20.)
    axUp.set_xlim(x0, xf)
...
def cornerPlotBothO(x, y, X2_N, X2_I):
    ...
    for i in range(4):
        axUp.plot([x0, xf], [levels1d[i], levels1d[i]],
                  color=colors[i], linewidth=0.5)
        axRi.plot([levels1d[i], levels1d[i]], [y0, yf],
                  color=colors[i], linewidth=0.5)
    axUp.set_ylim(0., 20.)
    axUp.set_xlim(x0, xf)
