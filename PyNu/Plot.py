...
class Plot:
    ...
    def __init__(self, analysis_output_file, analysis_input_file=False, directory=None):
    ...
    def Plot1D(self, all_plots=True, also_stats_only=False):
        if all_plots:
            if self.NumberOfPhysicsPars < 4:
                fig, ax = plt.subplots(nrows=1, ncols=self.NumberOfPhysicsPars)
                axis = [ax]
            else:
                ncols = 3
                nrows = self.NumberOfPhysicsPars // ncols
