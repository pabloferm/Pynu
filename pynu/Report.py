...
class Report:
    def __init__(
        ...
        self.doc = pylatex.Document(
            documentclass=doctype, geometry_options=geometry_options
        )
    ...
    def make_introduction(self):
        introduction = (
            ...
            + "using the "
            + self.source
            + "neutrino sources, measured by the "
            + self.det
            + self.detector_type
            + ".\n"
        )
        ...
        with self.doc.create(Section("Results")):
            with self.doc.create(Figure(position="h!")) as results_plot:
                results_plot.add_image(
                    self.directory + "/ResultPlotsMatrix.png", width="15 cm"
                )
    ...
    def make_nuisance(self):
        ...
        with self.doc.create(Section("Nuisance Parameters")):
            with self.doc.create(Figure(position="h!")) as results_plot:
                results_plot.add_image(
                    self.directory + "/NuisancePlots.png", width="15 cm"
                )
                results_plot.add_caption(
