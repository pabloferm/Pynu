...
class Plot:
    def __init__(self, analysis_output_file, analysis_input_file=False, directory=""):
    ...
    def colors(self):
        ...
        self.nuislevels_txt_1d = {1: r"$1\sigma$", 4: r"$2\sigma$"}
        ...
        for i, (item_row, values_row) in enumerate(self.PhysicsFlat.items()):
            for j, (item_col, values_col) in enumerate(self.PhysicsFlat.items()):
                ...
                """
                    ...
                    # print(h1)
                    ax[i, j].legend(h1, self.levels_txt_2d.values(), fontsize=8)
                    ax[i, j].set_xlabel(self.Format(item_col), fontsize=12)
    ...
    def NuisancePlots(self, all_plots=True, interpolate=True):
        ...
        if all_plots:
            if self.NumberOfNuisancePars < 5:
                fig, ax = plt.subplots(
                    nrows=1, ncols=self.NumberOfPhysicsPars, figsize=(14, 4)
                )
                axis = ax.flat
            else:
                ncols = 4
            ...
            for i, (item, values) in enumerate(self.NuisanceFlat.items()):
                ...
                x = np.linspace(prior_mu - 3 * prior_sig, prior_mu + 3 * prior_sig, 51)
    ...
    def Plot1D(self, all_plots=True, also_stats_only=False, interpolate=True):
        if all_plots:
            if self.NumberOfPhysicsPars < 4:
                fig, ax = plt.subplots(
                    nrows=1, ncols=self.NumberOfPhysicsPars, figsize=(12, 36)
                )
                axis = [ax]
            else:
                ncols = 3
    ...
    def Plot2D(self, all_plots=True, also_stats_only=False, interpolate=True):
        if self.NumberOfPhysicsPars < 2:
            ...
            for j in range(1, ncols - 1):
                ax[nrows - 2][j] = plt.subplot(
                    gs[nrows - 2, j], sharex=ax[nrows - 2][0]
                )
    ...
    def Format(self, string):
        ...
        if "Dm" in string:
            new_string += r"$\Delta m^2_{" + string[-2] + string[-1] + "}$ [eV$^2$]"
